...
export function render_groups(props){
    if(props){
        VK.Api.call('groups.get', {user_ids: userId, count: group_count, v:"5.131"}, function(r) {
            console.log(r.response.count)
            if (group_count > r.response.count){
                group_count = r.response.count
            }
            VK.Api.call('groups.getById', {group_ids: r.response.items, v:"5.131"}, function(r) {
                console.log(r)
...
class GroupsList extends React.Component{
    render(){
        return(
            <div>
                {/* <input type='range' id='group_value' name='Количество групп' min='5' max='500'/> */}
